...
from data_analysis import moving_average, process_regagent_rewards, process_attacker_rewards, process_defender_rewards
...
state_size_attackers, n_stage_actions, n_cyber_actions, n_disinfo_actions, stage_actions_dict, mal_contacts_dict, malagents_dict = \
    len(env.observation_spaces[f"malagent"]), env.action_spaces[f"malagent"][0].n, env.action_spaces[f"malagent"][1].n, \
    env.action_spaces[f"malagent"][2][f"malagent{malagent_example}"].n, env.stage_actions_dict , env.mcontacts_dict, env.malagents_dict
...
print("Attackers' state shape is", state_size_attackers, ", number of stage actions is", n_stage_actions, ", number of cyber actions is", 
      n_cyber_actions, " and number of disinformation actions is", n_disinfo_actions)
...
disinfo_opinion = 2 # initialised with environment
...
# Collect data
# Regular agents
regagent_total_rewards = np.zeros(number_of_episodes + 1)
regagent_action_rewards, regagent_opinion_rewards = np.zeros(number_of_episodes + 1), np.zeros(number_of_episodes + 1)
# Defenders
total_defender_rewards = np.zeros((number_of_episodes + 1, len(providers)))
defender_filter_rewards, defender_answer_rewards = np.zeros((number_of_episodes + 1, len(providers))), np.zeros((number_of_episodes + 1, len(providers)))
loss4_history, loss5_history = np.zeros((number_of_episodes + 1, len(providers))), np.zeros((number_of_episodes + 1, len(providers)))
# Attackers
total_attacker_rewards = np.zeros(number_of_episodes + 1)
reconnaissance_rewards, cyber_attack_rewards, disinfo_rewards, termination_rewards = \
    np.zeros(number_of_episodes + 1), np.zeros(number_of_episodes + 1), np.zeros(number_of_episodes + 1), np.zeros(number_of_episodes + 1)
...
for episode in range(1, number_of_episodes + 1):
    ...
    for timestep in range(1, n_steps+1): # one episode
        ...
        defenders_obs = env._monitor_requests_and_sn(actions) # Defender observations
    ...
    for agent_name, agent in malicious_agent.items():
        loss1, loss2, loss3 = agent.compute_a2c_loss(all_observations[agent_name][1:], all_actions[agent_name], \
                                                     episode_rewards[agent_name], gamma_ann1, gamma_ann2, gamma_ann3, entropy_coef)
    ...
    for agent_name, agent in service_providers.items():
        loss4, loss5 = agent.compute_a2c_loss(all_observations[agent_name][1:], all_actions[agent_name], \
                                              episode_rewards[agent_name], gamma_dnn1, gamma_dnn2, entropy_coef)
    ...
    if episode != 1 and episode % saving_freq == 0:
        ...
        regagents_df = pd.DataFrame({'total_rewards': regagent_total_rewards[1:episode+1],
                                     'action_rewards': regagent_action_rewards[1:episode+1],
                                     'opinion_rewards': regagent_opinion_rewards[1:episode+1]})
        regagents_df.to_csv(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-regagents-data.csv'), index = False)

        # Save attackers' data
        attacker_reward_loss_df = pd.DataFrame({'total_rewards': total_attacker_rewards[1:episode+1],
                                                'recon_rewards': reconnaissance_rewards[1:episode+1],
                                                'cyber_attack_rewards': cyber_attack_rewards[1:episode+1],
                                                'disinfo_rewards': disinfo_rewards[1:episode+1],
                                                'term_rewards': termination_rewards[1:episode+1],
                                                'loss1_history': loss1_history[1:episode+1],
                                                'loss2_history': loss1_history[1:episode+1],
                                                'loss3_history': loss1_history[1:episode+1]})
        # Save dataframes to csv
        attacker_reward_loss_df.to_csv(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-attacker-data.csv'), index = False)

        # Collect data
        defender_reward_loss_df = pd.DataFrame({'total_rewards_sp0': total_defender_rewards[1:episode+1,0],
                                                'total_rewards_sp1': total_defender_rewards[1:episode+1,1],
                                                'total_rewards_sp2': total_defender_rewards[1:episode+1,2],
                                                'filter_rewards_sp0': defender_filter_rewards[1:episode+1,0],
                                                'filter_rewards_sp1': defender_filter_rewards[1:episode+1,1],
                                                'filter_rewards_sp2': defender_filter_rewards[1:episode+1,2], 
                                                'answer_rewards_sp0': defender_answer_rewards[1:episode+1,0],
                                                'answer_rewards_sp1': defender_answer_rewards[1:episode+1,1],
                                                'answer_rewards_sp2': defender_answer_rewards[1:episode+1,2],
                                                'loss4_sp0': loss4_history[1:episode+1,0],'loss4_sp1': loss4_history[1:episode+1,1],
                                                'loss4_sp2': loss4_history[1:episode+1,2], 'loss5_sp0': loss5_history[1:episode+1,0],
                                                'loss5_sp1': loss5_history[1:episode+1,1], 'loss5_sp2': loss5_history[1:episode+1,2]})
        # Save dataframes to csv
        defender_reward_loss_df.to_csv(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-defender-data.csv'), index = False)
    ...
    if episode != 1 and episode % vis_freq == 0:
        ...
        if save_fig:
            plt.savefig(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-defender-score.png'))
            plt.clf()
        ...
        if save_fig:
            plt.savefig(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-defender-score2.png'))
            plt.clf()
        ...
        axs[0].legend(loc=(0.01,0.50), fontsize='x-small')
        if save_fig:
            plt.savefig(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-defender-loss.png'))
            plt.clf()
        ...
        plt.legend(loc=(0.01,0.50), fontsize='x-small')
        if save_fig:
            plt.savefig(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-attacker-score.png'))
            plt.clf()
        ...
        ax[2].grid()
        if save_fig:
            plt.savefig(os.path.join(save_path, f'ch{chapter}-exp{experiment}-{date}-{seed}-attacker-loss.png'))
            plt.clf()
