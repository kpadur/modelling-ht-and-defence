...
for agent_name, agent in regular_agents.items():
    ...
    action_checkpoint = torch.load(f'/regagent-parameters/{agent_name}_checkpoint_actions_2024-06-24_5281.pth')
    ...
    opinion_checkpoint = torch.load(f'/regagent-parameters/{agent_name}_checkpoint_opinions_2024-06-24_5281.pth')
...
for episode in range(1, number_of_episodes + 1):
    ...
    if episode != 1 and episode % vis_freq == 0:
        ...
        plt.grid()
        plt.legend(loc=(0.01,0.50), fontsize='x-small')
        # plt.savefig(os.path.join(output_dir, f'ch{chapter}-exp{experiment}-{date}-{seed}-trust.png'))
        # plt.clf()
